    ...
    <div className="min-h-[calc(100vh-4rem)]">
      <section className="max-w-5xl mx-auto px-5 sm:px-6 pt-12 sm:pt-20 pb-10 sm:pb-12">
        ...
        <p className="mt-5 text-grey-600 text-[15px] sm:text-[17px] max-w-xl leading-relaxed font-medium tracking-tight">
          여러 감정 표현을 한 편의 짧은 일기로 연결한 예시예요.
          <br />
          마음에 드는 문장은 클릭해서 복사해보세요.
        </p>
      </section>

      <div className="max-w-5xl mx-auto px-5 sm:px-6 pb-24 space-y-5">
        {error ? (
          <div className="bg-white border border-grey-100 rounded-3xl p-8 text-center">
            ...
            <p className="text-[14px] text-grey-500 mt-2 font-medium">
              {error}
            </p>
          </div>
        ) : isLoading ? (
          <>
            {Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="h-72 bg-white border border-grey-100 rounded-3xl animate-pulse"
              />
            ))}
          </>
        ) : samples.length === 0 ? (
          <p className="text-center text-grey-500 py-12 font-medium">
            아직 등록된 일기 샘플이 없어요
          </p>
        ) : (
          samples.map((sample) => (
...
function DiarySentenceRow({
  ...
  const category = sentence.category_slug
    ? categoryBySlug.get(sentence.category_slug)
    : null;
