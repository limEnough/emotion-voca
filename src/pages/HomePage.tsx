...
export function HomePage() {
  ...
  const preFilteredByCategoryAndSearch = useMemo(() => {
    ...
    return list.filter((e: Expression) => {
      if (categoryFilter !== "all" && e.category_slug !== categoryFilter)
        return false;
      if (q) {
        const haystack = [
          e.ko_pattern,
  ...
  const filtered = useMemo(() => {
    ...
    return preFilteredByCategoryAndSearch.filter(
      (e) => e.difficulty === difficultyFilter,
    );
        ...
        {error ? (
          ...
          <div className="grid gap-3 sm:gap-4 grid-cols-1 lg:grid-cols-2">
            {filtered.map((exp) => {
              ...
              if (!cat) return null;
              return (
                <ExpressionCard key={exp.id} expression={exp} category={cat} />
              );
            })}
          </div>
        )}
  ...
  return (
    <section className="max-w-5xl mx-auto px-5 sm:px-6 pt-12 sm:pt-20 pb-10 sm:pb-12">
      ...
      <p className="mt-5 text-grey-600 text-[15px] sm:text-[17px] max-w-xl leading-relaxed font-medium tracking-tight">
        ...
        한국어로는 자연스럽지만 영어로는 막막했던 그 감정들을 일기에 쓸 수 있는
        패턴으로 정리했어요.
      </p>
    </section>
  );
  ...
  return (
    <div className="grid gap-3 sm:gap-4 grid-cols-1 lg:grid-cols-2">
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="h-36 bg-white border border-grey-100 rounded-3xl animate-pulse"
        />
      ))}
    </div>
  );
